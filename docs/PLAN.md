...
## M10b stage 10 — opt-in CPU affinity: pin the LOAD, float the presenter (done)

**Scope:** the safe version of stage 9's rejected experiment. The frame clock
showed that sustained pacing (~5–7 fps) is limited by **CPU contention on the
flusher's core**, not by the present path (sub-ms). The scheduler's
`owner_cpu` is *not* a pin — `plan_switch` rewrites `owner_cpu = me` on every
switch and freely steals any Ready task onto any CPU. So a latency-critical
task has no way to stay on a core.

**What this adds.**
- `Task.pinned_cpu` (`u8::MAX` = unpinned). `plan_switch` skips a pinned task
  unless it is running on that task's pinned CPU, so the pin is real (the
  pinned task is never even *considered* by the wrong CPU).
- `scheduler::spawn_pinned(entry, prio, cpu)` — spawn a pinned task, with the
  stage-9 safety rule baked in: **refuse an offline target** (return `None`
  instead of creating a task whose context no CPU will claim, which is what
  caused the stage-9 null-context `#GP`).
- `plan_switch` honours the pin in the Ready scan.

**The measured result — and the design lesson.** Pinning the **flusher** to the
BSP made pacing dramatically **worse** (~1 fps): the BSP also runs the shell,
ticker, clock and the frame clock, and a pinned flusher can never be stolen to
a quieter core. Pinning the **frame clock (the load)** to a spare AP, and
letting the flusher **float** to whichever core is free, is what actually works:
pacing returns to ~8–11 fps *and* the load is off the busy BSP. So the rule is
**"pin the load, float the presenter"** — the steal behaviour that hurt the
pinned flusher is exactly what rescues the floating one. Both are now recorded
so the next person does not "fix" the flusher by pinning it.

**Honest scope.** This is a real, safe improvement (~5 → ~8–11 fps by moving the
load off the presenter) plus the affinity *mechanism* the presenter will need
later. It is NOT a 1000 FPS result: on this 2-cpu TCG guest the present path is
sub-ms but the frame cadence is still scheduler-bound, and the emulated
environment caps what the numbers mean (see stage 9). The mechanism now exists
to build on; the remaining pacing gap is a scheduler/CPU-budget problem.

Suites: `test-latency.ps1`, `test-gpu.ps1` ×2, `test-smp.ps1`, `test-avx.ps1`,
`test-fs.ps1`, `test-shell.ps1` — all exit 0 (smp is the key one: it exercises
the scheduler directly and the new pin path). `build.ps1` clean.
